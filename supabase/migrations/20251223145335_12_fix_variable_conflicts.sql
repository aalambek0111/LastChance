/*
  ...
  1. Changes
    - Remove unused variables that conflict with column names
...
BEGIN
  ...
  SELECT
    ...
    (SELECT l.id FROM leads l WHERE l.organization_id = NEW.id AND l.email = booking_data.lead_email LIMIT 1),
  ...
  SELECT NEW.id, l.id, l.channel, 'open', team_member_ids[1], now() - interval '10 minutes'
  FROM leads l
