/*
  # Fix Demo Data Seeding Trigger

  1. Changes
    - Fix array handling in seed_organization_demo_data function
    - Remove incorrect array subscript usage
    - Properly collect team member IDs after insertion
*/

CREATE OR REPLACE FUNCTION seed_organization_demo_data()
RETURNS TRIGGER AS $$
DECLARE
  tour_ids uuid[];
  ...
  i integer;
BEGIN
  -- Get all tour IDs for this organization
  ...
  SELECT NEW.id, id, channel, 'open', team_member_ids[1], now() - interval '10 minutes'
  FROM leads
  WHERE organization_id = NEW.id
